...
import { parseTakeAndCloseTxs } from "../utils/takeSwapUtils";
...
export async function createTakeAndCloseSwapInstructions(
  ...
  try {
    ...
    if (!acceptedBid) {
      ...
      if (takerNftStd == "core") {
        ...
        if (takerNftStd === "native") {
          ...
          const takeIx = await program.methods
            .takeSwap(bidToscBid(bid), n, traitIndex ?? null, traitsProofPk)
            .accountsStrict({
              swapDataAccount,
              swapDataAccountTokenAta,
          ...
          const takeIx = await program.methods
            .takeSwap22(bidToscBid(Data.bid), n, traitInd, traitsProofPk)
            .accountsStrict({
              swapDataAccount,
              swapDataAccountTokenAta,
    ...
    if (
      ...
      if (signer === taker)
        claimIxs.push(closeWSol(taker, taker, takerTokenAta));
